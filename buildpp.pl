...
#The preprocessor to use if it is enabled
my $preProcessor:shared = "";
#Regular expression that if the file matches causes the pre processor to be used
my $usePreProcessor = "";
...
sub parseFile
{
  ...
  #Test if we should use a pre processor
  if (length($usePreProcessor)>0 &&
      $filePath =~ /$usePreProcessor/){
      open ($file, "$preProcessor $cflags $incDirs $filePath|") || die "Unable to use the pre processor '$preProcessor $filePath'";
  }
  else {
      open ($file, "<$filePath") 
